    ...
    <div class="">
        ...
        <div class="flex space-x-2">
            @foreach($categories as $category)
                <button class="flex w-full justify-center text-base text-white bg-orange-500 rounded-full border border-gray-300 py-2 px-4 shadow shadow-md hover:scale-105 duration-500" wire:click="selectCategory('{{$category}}')">{{$category}}</button>
            @endforeach
        </div>
    </div>
